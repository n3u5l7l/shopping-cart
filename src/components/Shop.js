...
function Shop ({handleShoppingCart: setShoppingCart}){
    ...
    const updateCart = (nutType, nutCost, nutImg) => {setShoppingCart((prevState) => Object.assign({}, prevState, prevState[nutType] && prevState[nutType].amount >= 0  ? {[nutType]: {...prevState[nutType], amount: Number(prevState[nutType].amount)+1, total: Number(prevState[nutType].amount) * Number(prevState[nutType].cost)}, totalCost: prevState["totalCost"] + nutCost, totalItem: prevState["totalItem"] + 1}: {[nutType]: {amount: 1, cost: nutCost, image: nutImg}, totalCost: prevState.totalCost ? prevState.totalCost + nutCost: nutCost, totalItem: prevState.totalItem ? prevState.totalItem + 1: 1} )); console.log("LoL");};
    let content = (
        NutInfos.map((nut, index) => {
            ...
            <div className="item-container" key={index}>
                ...
                <div className="item-info">
                    ...
                    <button onClick={() => updateCart(nut.type, Number(nut.cost), nut.image)}>Add to cart</button>
                </div>
            </div>
            )
        }
